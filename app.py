...
def create_report(score):
    ...
    if score <= 4:
        rows = [
            ["Mon", "Autophagy", "Hydration focus. Start with Miso soup."],
            ["Tue", "Microbiome", "Eat Natto at dinner to support mucosa."],
            ["Wed", "Enzyme", "Seaweed side dish. Support digestion."],
        ...
        rows = [
            ["Mon", "Autophagy", "16:8 Fasting. Miso & Seaweed."],
            ["Tue", "Microbiome", "Mix Natto with Okra/Kimchi."],
            ["Wed", "Enzyme", "Seaweed Salad. Activate Porphyranase."],
            ["Thu", "Nootropic", "2g Ippodo Matcha. L-Theanine."],
            ["Fri", "Omega-3", "Fatty fish + 1g supplement."],
            ["Sat", "Metabolism", "HIIT Session. Glycolysis."],
            ["Sun", "Rest", "Hot Bath + 5min cold shower."]
        ]
...
def home():
    ...
    <head>
        ...
        <style>
            ...
            h1 {{ font-size:6.5rem; letter-spacing:30px; color:var(--neon); font-weight:100; text-shadow:0 0 35px var(--neon); cursor:pointer; }}
            ...
            .q-item {{ margin-bottom:15px; display:flex; align-items:center; font-size:1.1rem; color:#ccc; }}
            input[type="checkbox"] {{ transform:scale(1.6); margin-right:20px; accent-color:var(--neon); }}
            button {{ background:transparent; color:var(--neon); border:1px solid var(--neon); padding:20px 70px; font-weight:bold; cursor:pointer; letter-spacing:7px; transition:0.6s; margin-top:40px; text-transform:uppercase; }}
            ...
            .legal-links {{ margin-top:30px; font-size:0.7rem; color:#333; }}
            .legal-links a {{ color:#333; text-decoration:none; margin:0 10px; }}
        </style>
    </head>
    <body>
        ...
        <div id="page2" class="screen">
            <div class="card">
                <div style="color:var(--neon); font-size:0.7rem; letter-spacing:5px; margin-bottom:20px;">02 // DIETARY INPUT</div>
                ...
                <div style="color:var(--neon); font-size:0.7rem; letter-spacing:5px; margin-top:30px; margin-bottom:15px;">02b // INVERSE FACTOR</div>
                ...
                <div class="legal-links"><a href="/about">ABOUT US</a> | <a href="/legal">COMMERCE DISCLOSURE</a></div>
            </div>
            <div class="disclaimer">ADVICE ONLY. NOT A MEDICAL DIAGNOSIS.</div>
        </div>
        <div id="page3" class="screen">
            <div class="card" style="text-align:center;">
                <h2 style="letter-spacing:10px; font-weight:100; font-size:2rem; color:var(--neon);">BLUEPRINT READY</h2>
                <div style="font-size: 1.5rem; margin-top: 20px;">Score: <span id="dispScore" style="color:var(--neon); font-weight:bold;">0</span>/8</div>
        ...
        <script>
            ...
            let w, h, orbs = [], state = "dance", debug = {debug_js};
            function init() {{
                w = canvas.width = window.innerWidth; h = canvas.height = window.innerHeight;
                orbs = []; for(let i=0; i<15; i++) orbs.push({{x:Math.random()*w, y:Math.random()*h, r:Math.random()*200+100, v:{{x:(Math.random()-0.5)*0.5, y:(Math.random()-0.5)*0.5}}}});
            }}
            function draw() {{
                ctx.clearRect(0,0,w,h);
                orbs.forEach(o => {{
                    if(state === "dance") {{ o.x += o.v.x; o.y += o.v.y; if(o.x<0||o.x>w) o.v.x*=-1; if(o.y<0||o.y>h) o.v.y*=-1; }}
                    else {{ o.x += (w/2 - o.x) * 0.02; o.y += (h/2 - o.y) * 0.02; }}
...
def create_checkout_session():
    ...
    try:
        session = stripe.checkout.Session.create(
            line_items=[{'price_data': {'currency': 'usd', 'product_data': {'name': 'Longevity Blueprint'}, 'unit_amount': 500}, 'quantity': 1}],
            mode='payment',
...
def legal():
    ...
    <p><b>販売業者:</b> 佐久間稜<br><b>所在地:</b> 北海道札幌市北区北13条西8丁目 北海道大学大学院<br><b>価格:</b> $5.00<br><b>連絡先:</b> ryo1ryo2-1103@outlook.jp</p>
    <a href="/" style="color:#39FF14;">BACK</a></body>"""
